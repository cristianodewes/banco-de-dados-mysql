...
SELECT idUsuario AS Usuário, titulo AS Título, qtdcancoes AS "Quantidade de Músicas",
	idestado AS Estado, DATAcriacao AS "Data de criação", Dataexclusao AS "Data de exclusão" 
...
INSERT INTO usuario (IdUsuario, NomeUsuario, NomeCompleto, senha, Data_nac, sexo,
  cod, Pais_idPais, IdTipoUsuario)
VALUES ('20', 'novousuariodespotify@gmail.com', 'Elmer Santos', 'S4321m', '1991-11-15',
 'M', 'B4129ATF', '9', '1');
 
 SELECT * FROM cancao;
  SELECT * FROM cancao WHERE idcancao IN (SELECT Idcancao FROM generoxcancao WHERE idGenero = 9);
 
DELETE FROM cancao WHERE idcancao 
IN (SELECT Idcancao FROM generoxcancao WHERE idGenero = 9);

DELETE cancao FROM generoxcancao WHERE idcancao 
IN (SELECT Idcancao FROM generoxcancao WHERE idGenero = 9);
